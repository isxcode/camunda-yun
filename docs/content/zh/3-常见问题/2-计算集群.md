...
```bash
cd ~/zhishuyun-agent/spark-min/logs
...
```bash
cd ~/zhishuyun-agent/spark-min/sbin
...
```wikitext
检查`~/zhishuyun-agent/spark-min/conf`下的`spark-defaults.conf`文件中配置是否正确
```
...
```bash
vim ~/zhishuyun-agent/spark-min/conf/spark-defaults.conf
```
...
```bash
vim ~/zhishuyun-agent/spark-min/conf/spark-env.sh
```
...
```log
stop.sh:行2: $'\r'：未找到命令
stop.sh: 第 5 行：cd: $'/tmp/zhishuyun/bin\r': 没有那个文件或目录
