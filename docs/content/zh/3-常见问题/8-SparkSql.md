...
```wikitext
这是因为spark的standalone集群版本，和camunda-yun中spark-min中自带的版本冲突，需要将两边的版本保持一致
将用户自己的spark安装包复制到spark-min中
```

```bash
cp -r /opt/spark/* /home/ispong/camunda-yun/spark-min/
rm /home/ispong/camunda-yun/spark-min/conf/spark-env.sh
rm /home/ispong/camunda-yun/spark-min/conf/spark-defaults.conf
```
